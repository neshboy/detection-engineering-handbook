...
The `-enc` flag (short for `-EncodedCommand`, one of the parameter-abbreviation forms PowerShell accepts) tells the PowerShell engine to base64-decode its argument into a script before executing it — the attacker's actual command, in this case, is a `DownloadString` cradle. This is a well-worn technique: MITRE ATT&CK T1059.001 (PowerShell), a sub-technique of T1059 (Command and Scripting Interpreter) under tactic TA0002 (Execution), commonly paired with T1027 (Obfuscated Files or Information) under TA0005 (Defense Evasion) for the encoding itself. Every technique and sub-technique ID cited in this book follows MITRE's own ATT&CK documentation (MITRE ATT&CK, "Command and Scripting Interpreter: PowerShell," T1059.001: https://attack.mitre.org/techniques/T1059/001/) as the canonical, versioned source for a technique's scope and observed procedure examples — this book names an ID as a pointer into that living catalog, not a substitute for reading it.
